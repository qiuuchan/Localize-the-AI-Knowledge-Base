...
> **v2.2.0 = Polish & Hardening 周期收口**(工单池 `docs/superpowers/plans/2026-09-01-v2.2-ticket-pool.md` T01-T24 全收官)。两条主线:①公开仓叙事一致性与卫生收口(sanitizer STRIP 剥离 / v210 报告公开 / 文档口径统一);②技术短板补齐(上下文工程 / LLM-as-judge 评测 / ADR-0003+0004)。公开仓历史 = 9 个整理 commit(脱敏管线),完整开发历史可追溯。
...
- **真实评测落地(2026-09-01,23 条全量,成本 ≈¥0.72)**:judge 口径任务完成率 22/23(95.65%)vs 同答案关键词口径 23/23(100%)——双口径差异 = judge 抓出 1 条 keyword 假阳性(「今天深圳有什么餐饮新闻」回答"未能获取"仍被"非空即过"放行);v210 自标的 2 条「缺 % 字符」假阴性在语义口径下必然判完成(机制确认);judge 23/23 纯 JSON 解析 0 降级;工具选择 22/23 持平 v210 但失败用例不同(采样方差再验证,三轮 87/87/95.65);新盲区:模型直接引用文档现成数值绕过 calculator(v2.2 后候选);落库 `eval_runs` `agent-llm-2026-09-01`
...
- 新增 `docs/REVIEW-GUIDE.md`:评审 5 分钟导览(看什么文件、每个亮点对应的代码锚点)
...
- `README.md` 全文重写为公开版结构:一句话定位 + mermaid 架构图(5 容器)+ 实测工程指标表(300 pytest / 13 vitest / 50 golden-QA / 36 API 端点 / 双平台)+ 设计决策摘要(便携/低依赖/降级/评测驱动/成本工程);指标数字均来自 2026-08-26 本机 grep/wc 实测;许可改为 MIT(LICENSE 文件待 T02 随公开仓一并补齐)
